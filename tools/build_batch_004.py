"""Build fully developed frequency cards for ranks 201-500."""
...
OUT = ROOT / "data" / "canonical" / "frequency-0201-0500.jsonl"
DATA_FILES = tuple(
    ROOT / "tools" / f"batch_004_{start:04}_{start + 49:04}.txt"
    for start in range(201, 501, 50)
)
...
def build() -> list[dict]:
    ...
    if ranks and ranks != list(range(201, 201 + len(ranks))):
        raise ValueError("Authored ranks must be consecutive from 201")
    return rows


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--require-through", type=int, default=200)
    args = parser.parse_args()
    rows = build()
    if args.require_through >= 201:
        expected_count = args.require_through - 200
    ...
    print(f"Wrote {len(rows)} cards through rank {200 + len(rows)} to {OUT}")
